...
class Payworks extends EventEmitter {
  ...
  auth(_params, options, callback) {
    ...
    let schema = {
      ...
      XID: joi.string(),
      CAVV: joi.string(),
      STATUS_3D: joi.string(),
      VERSION_3D: joi.number(),
      customer_ref1: joi.string().max(30),
      customer_ref2: joi.string().max(16),
      customer_ref3: joi.string().max(30),
      customer_ref4: joi.string().max(30),
      customer_ref5: joi.string().max(30),
    }
    ...
    if (params.ECI && !params.XID && !params.CAVV && !params.STATUS_3D && !params.VERSION_3D) throw new Error('No available')
